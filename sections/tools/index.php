                ...
                if (!$Err) {
                    if (!is_numeric($_REQUEST['id'])) {
                        $DB->query("INSERT INTO permissions
                                            (Level,Description,Name,`Values`,DisplayStaff,IsUserClass,MaxSigLength,MaxAvatarWidth,MaxAvatarHeight,Color)
                                     VALUES ('" . db_string($Level) . "','" . db_string($Description) . "','" . db_string($Name) . "','" . db_string(serialize($Values)) . "','" . db_string($DisplayStaff) . "','" . db_string($IsUserClass) . "','" . db_string($MaxSigLength) . "','" . db_string($MaxAvatarWidth) . "','" . db_string($MaxAvatarHeight) . "','" . db_string($Color) . "')");
                    } else {
                        $DB->query("UPDATE permissions SET Level='" . db_string($Level) . "',Description='" . db_string($Description) . "',Name='" . db_string($Name) . "',`Values`='" . db_string(serialize($Values)) . "',DisplayStaff='" . db_string($DisplayStaff) . "',MaxSigLength='" . db_string($MaxSigLength) . "',MaxAvatarWidth='" . db_string($MaxAvatarWidth) . "',MaxAvatarHeight='" . db_string($MaxAvatarHeight) . "',Color='" . db_string($Color) . "' WHERE ID='" . db_string($_REQUEST['id']) . "'");
