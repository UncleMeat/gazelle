...
$images = scandir(SERVER_ROOT . '/static/common/flags/iso16', 0);
            ...
            <form action="tools.php" method="post">
                ...
                <td class="right">
                    <span id="lang_image<?=$id?>"> 
                        <img style="vertical-align: bottom" title="<?=$flag_cc?>" src="http://<?=SITE_URL?>/static/common/flags/iso16/<?=$flag_cc?>.png" />
                    </span>
                </td>
                <td>
                    <span style=" ">
                        <select id="flag_image<?=$id?>" name="flag" onchange="change_image('#lang_image<?=$id?>', '#flag_image<?=$id?>');">
                            ...
                            <? foreach ($images as $key => $value) { 
                                    if (strlen($value)==6){     
                                        ...
                                        <option value="<?= display_str($value) ?>" <? if($flag_cc==$value) echo 'selected="selected"';?>><?= $value ?></option>
                            <?      }
                               }            
                            ?>
                        </select>
                ...
                <td>
                    <input type="checkbox" name="active" value="1" <? if($active) echo 'checked="checked"';?> />
                </td>           
                <td>
                    <input type="submit" name="submit" value="Edit" />
                    <input type="submit" name="submit" <? if(!isset($_GET['del'])) echo 'disabled="disabled"';?> value="Delete" title="To enable the delete button use the &del=1 parameter in the url, but normally you should just deactivate a language option" />
                </td>
            </form>
            </tr>
<? } ?>        
